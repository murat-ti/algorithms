...
function solution($A) {
    $min_value = $min_index = PHP_INT_MAX;
    ...
    for($i=0; $i<count($A); $i++) {
        ...
        for($j=$i+1; $j<count($A); $j++) {
            $total += $A[$i] + $A[$j];
            //echo $A[$i].' => '.$total / $start.PHP_EOL;
            if($total / $start < $min_value) {
                $min_value = $total / $start;
                $min_index = $i;
            }
            $start++;
        }
...
that, given a non-empty array A consisting of N integers, returns the starting position of the slice with the minimal average. If there is more than one slice with a minimal average, 
